...
export const Analytics = {
  ...
  // Fired on the /thankyou page view — this is the CONVERSION signal to mark as
  // a key event/conversion in GA4 and to hook ad platforms onto. Distinct from
  // waitlist_joined (which fires on submit, before the redirect completes).
  waitlistConfirmed: ({ position, foundingFamily } = {}) => trackEvent('waitlist_confirmed', {
    waitlist_position: position ?? undefined,
    founding_family: foundingFamily ? 'yes' : 'no',
    // GA4-recommended ecommerce-style value so it can power ROAS/attribution.
    currency: 'AUD',
    value: 1,
  }),
