...
// 1c. FOUNDING INVITE — sent to a waitlister when admin opens founding signups.
// Carries their unique /join?invite=<token> link + the offer terms.
export function foundingInviteEmail({ name, joinUrl, offer }) {
  const bullets = (offer?.bullets || [])
    .map(b => `<p style="margin:6px 0;">✅ ${b}</p>`)
    .join('')
  return baseTemplate({
    title: `You're invited — ${offer?.headline || 'founding-family offer'}`,
    previewText: `Your founding-family spot is ready. One month free, then $19.99/mo.`,
    content: `
      <h1>You're invited to join MyMathsHero 🎉</h1>
      <p>Hi ${name || 'there'},</p>
      <p>Thanks for joining our waitlist — we're opening the doors to our
      <strong>founding families</strong>, and your spot is ready.</p>

      <div class="highlight-box">
        <p style="margin:0;font-weight:800;color:#1B2B4B;font-size:16px;">
          🎁 ${offer?.headline || 'One month free, then founding pricing'}
        </p>
      ...
      <div class="divider"></div>
      <p style="text-align:center;">
        <a href="${joinUrl}" class="btn">Claim your founding spot →</a>
      </p>
      <p style="text-align:center;font-size:12px;color:#94A3B8;">
        This invite link is just for you — please don't share it.
      </p>
    `,
    footerText: `You're receiving this because you joined the MyMathsHero waitlist.`,
  })
}
