...
export async function solveBlind(q, { model = VERIFIER_MODEL } = {}) {
  ...
  const prompt = [
    ...
    `First decide: can this be answered from the text alone, or does it REQUIRE seeing a picture that isn't provided (e.g. "how many parts are shaded?" with no data given)?`,
    `Then, if it is answerable, solve it and choose the single correct option.`,
    `Show NO working. Respond with ONLY compact JSON: {"answer":"<exact option text>","confident":true|false,"needsImage":true|false}`,
    ``,
    `Question: ${q.question}`,
    `Options:`,
    opts,
...
export async function verifyQuestion(q, { model = VERIFIER_MODEL, double = true } = {}) {
  ...
  // Image-dependent (e.g. "how many parts are shaded?" with no data in text). The
  // student can never answer this — a broken question, not a wrong answer, so
  // inOptions:false routes it to deactivation in the re-check loop.
  if (v.needsImage) {
    return { status: 'suspect', verifierAnswer: v.answer, inOptions: false, reason: 'requires-unshown-visual' }
  }
