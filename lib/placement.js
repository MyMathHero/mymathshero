...
/**
 * Validate + shape an admin manual override (report §5). Pure so it can be
 * unit-tested. Returns { gradeChange?: int, override: {...} } or { error }.
 *
 * `grade` (enrolled year) and `level` (learning level the engine should treat
 * the student as working at) are both optional 0–12 ints. At least one must be
 * present. We DON'T rewrite skill_scores — overriding grade re-runs the AI
 * recommender against the new grade, preserving AI recommendations.
 */
export function buildLevelOverride({ grade, level, reason, by } = {}, existing = {}) {
  const toInt = (v) => {
    if (v === undefined || v === null || v === '') return undefined
    const n = Math.round(Number(v))
    if (!Number.isFinite(n) || n < 0 || n > 12) return null // null = invalid
    return n
  }
  const g = toInt(grade)
  const lvl = toInt(level)
  if (g === null || lvl === null) return { error: 'grade and level must be integers 0–12' }
  if (g === undefined && lvl === undefined) return { error: 'provide grade and/or level to override' }

  const override = {
    ...(g !== undefined ? { grade: g } : {}),
    ...(lvl !== undefined ? { level: lvl } : {}),
    reason: typeof reason === 'string' ? reason.trim().slice(0, 500) : '',
    by: typeof by === 'string' ? by.slice(0, 128) : 'admin',
    at: new Date(),
    previousGrade: existing.grade,
  }
  return { gradeChange: g, override }
}
