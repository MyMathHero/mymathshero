...
// Top grade the adaptive diagnostic / engine will climb to.
export const MAX_DIAGNOSTIC_GRADE = 12

/**
 * Adaptive-diagnostic climb decision. Given the results of the questions just
 * answered at a probe grade, decide whether to serve a HARDER stage (one grade
 * up) or stop. Pure + unit-tested.
 *
 * Rule: keep climbing while the student is clearly handling the level — at least
 * `minCorrect` of the stage's questions correct AND accuracy >= `threshold`.
 * Stops at MAX_DIAGNOSTIC_GRADE. `stageResults` = [{ correct }].
 */
export function shouldClimb(stageResults = [], currentGrade = 0, { threshold = 0.7, minCorrect = 2, maxGrade = MAX_DIAGNOSTIC_GRADE } = {}) {
  if (currentGrade >= maxGrade) return false
  const rs = Array.isArray(stageResults) ? stageResults : []
  if (rs.length === 0) return false
  const correct = rs.filter(r => r && r.correct).length
  if (correct < minCorrect) return false
  return correct / rs.length >= threshold
}
