...
export default function AvatarPage() {
  ...
  async function choose(id) {
    ...
    if (id === selected) return // no change → free, nothing to do
    // Changing costs coins — block early if they can't afford it.
    if ((coins || 0) < changeCost) {
      showMessage(`You need ${changeCost} coins to change your Hero`, 'error')
      return
    }
    const prev = selected
    ...
    try {
      ...
      if (data.success) {
        ...
        showMessage(
          data.coinsSpent
            ? `${getCharacter(id)?.name} selected! −${data.coinsSpent} 🪙`
            : `${getCharacter(id)?.name} selected! ✅`,
          'success'
        )
      } else {
        setSelected(prev)
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary, #F0F4F8)' }}>
      ...
      <div style={{ padding: 16 }}>
        <p style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-muted, #94A3B8)', textTransform: 'uppercase', letterSpacing: '0.5px', margin: '4px 4px 12px' }}>
          ✨ Pick a character <span style={{ color: '#C49A1A', textTransform: 'none' }}>· {changeCost} 🪙 per change</span>
        </p>
